...
with warnings.catch_warnings():
    ...
    from batchSeqs import batchByLength, Batcher, StreamingBatcher
    from segment_functions import * # lcs/1, getExponent/1
...
def get_context_vectors(clustering, context_instances, context_vocab, nDim=20, panels=20, drc=False):
    ...
    model = build(nVocab, clustering.nClustersUsed(), panels, nDim, directional=drc)
    ...
    for epoch in range(5):
        gen = streamingGen(context_instances, clustering.lem_2_wf_2_cluster, context_vocab)
    ...
    full_context_matrix = np.array([[0.0]*nDim]*(len(context_vocab)+clustering.nClustersUsed()))
    ...
    for cluster in range(clustering.nClustersUsed()):
