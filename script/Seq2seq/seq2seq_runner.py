...
def handle_preparation_flags(flags):
  ...
  print("Set checkpoint dir", checkpoint_dir, "prefix", checkpoint_prefix)
...
def prepare_data(flags, hparams):
  ...
  if hparams.checkpoint_to_restore:
    ...
    editMap(os.path.join(flags.work_dir, 'vocab_map.tsv'), flags.src_length, flags.targ_length)
  ...
  # Get train/dev/test without restoring an old model.
    ...
    with open(os.path.join(flags.work_dir, 'vocab_map.tsv'),
      ...
      split_objects = dataloader.get_splits(
          data_files, flags.data_format, vocab_map_file, hparams,
          restore=hparams.checkpoint_to_restore, src_length=flags.src_length, trg_length=flags.targ_length)
...
def editMap(mapF, src_length, targ_length):
  print("Editing vocab map to sequence lengths", src_length, targ_length)
  with open(mapF) as mapFH:
    lines = list(mapFH)
  if src_length > 0:
    for ind, li in enumerate(lines):
      if li.startswith("src_max_len_seq"):
        break
    lines[ind] = "src_max_len_seq\t%d\n" % (src_length,)
  if targ_length > 0:
    for ind, li in enumerate(lines):
      if li.startswith("trg_max_len_seq"):
        break
...
def validate_held_out(pred_filename, model, best_checkpoint_path, dev=True, losses=False):
    ...
    if losses:
        base_wf_tags_2_losses = model.validate_forced(dev=dev)
...
def run(flags, mode='normal'):
  ...
  model.dev_acc, model.test_acc = None, None
  automation_rate_dev, automation_rate_test = None, None
  if flags.dev:
    # Validate on dev set.
  ...
  if flags.test:
    if get_losses:
        sys.stdout.write('\t\tdev acc: {}\n'.format(model.dev_acc))
        sys.stdout.write('\tCalculating losses for all base,wf tuples\n')
        sys.stdout.flush()
