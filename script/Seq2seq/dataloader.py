...
class DataFormat(enum.Enum):
  ...
  def is_inflection(self):
    return self in (DataFormat.INFLECTION, DataFormat.INFLECTION_IC)
...
def preprocess_instance(data_format, instance
                       ) -> typing.Optional[_PreprocessedInstance]:
  ...
  elif data_format == DataFormat.INFLECTION:
...
def _get_tensors_for_split(split_indices, lemmata, src_tensor, trg_tensor,
                           src_feat_tensor, trg_feat_tensor,
                           inflection_class_tensor,
                           data_format) -> _SplitTensors:
...
def load_restore_friendly_dataset(data_files, data_format, vocab_map_file, src_length=None, trg_length=None,
                                 ) -> _DatasetObjects:
  ...
  if src_length is not None:
    src_max_len_seq = src_len
  if trg_length is not None:
    trg_max_len_seq = trg_length
...
def _vectorize_tensors(corpus_srcs, corpus_trgs, corpus_src_feats,
                       corpus_trg_feats, corpus_inflection_classes, data_format,
                       src_language_index, trg_language_index,
                       src_feature_index, trg_feature_index,
                       inflection_class_index):
...
def _pad_tensors(src_tensor, trg_tensor, src_feat_tensor, trg_feat_tensor,
...
def load_dataset(data_files, data_format, vocab_map_file, src_length=None, trg_length=None) -> _DatasetObjects:
  ...
  if src_length is not None:
    src_max_len_seq = max(src_max_len_seq, src_length)
    trg_max_len_seq = max(trg_max_len_seq, trg_length)
...
def get_splits(data_files, data_format, vocab_map_file, hparams, restore=None, src_length=None, trg_length=None,
              ) -> _SplitObjects:
  ...
  if restore:
    dataset_objects = load_restore_friendly_dataset(data_files, data_format,
                                                    vocab_map_file, src_length=src_length, trg_length=trg_length)
  else:
    dataset_objects = load_dataset(data_files, data_format, vocab_map_file, src_length=src_length, trg_length=trg_length)
...
def prepare_for_transformer(flags, hparams, all_data,
                            vocab_map_file) -> _TFSplitObjects:
  ...
  print("Using language index", src_language_index)
  print("Using targ index", trg_language_index)
...
def _get_transformer_language_index(hparams, vocab_map_file, token_list=None,
                                    max_len=None):
...
def reformat_split_for_transformer(dataset, flags, all_data: _SplitObjects):
  ...
  mls = max([len(s.split()) for s in srcs])
  if flags.src_length > -1:
    mls = flags.src_length
  return srcs, trgs, mls
